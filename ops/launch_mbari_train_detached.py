#!/usr/bin/env python3
"""Launch the MBARI training orchestrator detached from the terminal.
...
ROOT = Path(os.environ.get("MBARI_PROJECT_ROOT", Path(__file__).resolve().parents[1])).resolve()
ORCHESTRATOR = ROOT / "mbari_train.py"
RESULTS = ROOT / "nn_results"
PYTHON = Path(os.environ.get("MBARI_PYTHON", sys.executable))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Start mbari_train.py detached, writing output under nn_results."
    )
    parser.add_argument(
        "--log",
        default=None,
    ...
    parser.add_argument(
        "--project-root",
        default=str(ROOT),
        help="project root to run from; also exported as MBARI_PROJECT_ROOT",
    )
    parser.add_argument(
        "--mem-guard-gb",
        type=float,
...
def main(argv: list[str] | None = None) -> int:
    ...
    env = os.environ.copy()
    env["MBARI_PROJECT_ROOT"] = str(root)
