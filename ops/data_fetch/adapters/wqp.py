"""Water Quality Portal (waterqualitydata.us) — expanded statewide CA bacteria.

Pulls fecal-indicator-bacteria results (E. coli, Enterococcus, fecal coliform) for the
whole state, chunked by YEAR so each request is bounded and the run is resumable. CSV
"narrowResult" profile. This expands the small Lover's Point/storm-drain WQP extract to
statewide coverage.
"""
...
CHARACTERISTICS = ["Escherichia coli", "Enterococcus", "Fecal Coliform"]
DEFAULT_START_YEAR = 2018


class WqpAdapter(Adapter):
    def iter_chunks(self, start: Optional[str], end: Optional[str]) -> Iterator[dict]:
        ...
        for yr in range(y0, y1 + 1):
            for char in CHARACTERISTICS:
                yield {"key": f"{yr}_{char.replace(' ', '')}", "year": yr, "characteristic": char}
