# Monterey Bay AI Lab Local Environment Ops

This directory contains setup artifacts for isolated local Monterey Bay AI Lab analysis environments on the RTX 5090 workstation.
...
- `mbal-gpu`: PyTorch CUDA, XGBoost GPU, core tabular ML, experiment tracking.
...
- `create_mbal_envs.ps1`: creates or updates isolated Python virtual environments.
...
- `requirements-mbal-gpu.txt`: packages for GPU forecasting and tabular ML.
...
- `launch_mbal_train_detached.py`: starts long training runs detached from the terminal.
...
- `requirements-mbal-gpu.txt` / `requirements-geo-ocean.txt` / `requirements-llm-local.txt`: dependency specs for the GPU, geo/ocean, and local-LLM paths.
...
```powershell
.\ops\create_mbal_envs.ps1 -EnvRoot D:\ai-envs
.\ops\verify_gpu_stack.ps1 -EnvRoot D:\ai-envs -EnvName mbal-gpu
```

## Safety

These scripts do not modify existing Monterey Bay AI Lab analysis scripts. They create or update only the selected virtual environments and install packages inside those environments.
...
For the production handoff gate, see `..\PRODUCTION_READINESS.md`, `..\MBAL_PRODUCTION_LAKEHOUSE_CONTRACTS.md`, and the portable smoke configurations in this directory.
...
If `pytest` is missing, install the GPU/tooling requirements or install `pytest` directly:
