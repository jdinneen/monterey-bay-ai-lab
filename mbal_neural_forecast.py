#!/usr/bin/env python3
"""
MBAL M1 neural forecasting harness (concurrency-safe).
...
Models (Nixtla neuralforecast): nhits, patchtst.  Interpretable: nhits (basis decomposition).
"""
...
from mbal_forecast_v2 import load_source, build_hourly_matrix
from mbal_core import apply_physical_quality_filters
from mbal_deep_models import parse_mbal_feature_semantics
from mbal_split_contracts import (
    SplitContract,
    build_split_contract as build_shared_split_contract,
    write_split_contract,
)
from ops.gpu_admission import enforce_gpu_admission, estimate_neural_forecast_mib
from neuralforecast.common._base_model import BaseModel

PROJECT_ROOT = Path(os.environ.get("MBAL_PROJECT_ROOT", Path(__file__).resolve().parent)).resolve()
PARQUET = os.environ.get(
    "MBAL_SOURCE_PARQUET",
    str(PROJECT_ROOT / "mbal_history" / "opendap"),
)
HORIZONS = [1, 6, 24, 72, 168]
FUTR = [
    "hour_sin", "hour_cos", 
    "doy_sin", "doy_cos",
    "m2_tide_sin", "m2_tide_cos",
    "k1_tide_sin", "k1_tide_cos"
]  # calendar & physics proxies, known into the future
...
CACHE_VERSION = "v4_physics_guided_tidal_futr"
LAKEHOUSE = Path(os.environ.get("MBAL_LAKEHOUSE_DIR", PROJECT_ROOT / "lakehouse"))
...
def file_fingerprint(path: str | Path) -> dict:
    ...
    return {
        ...
        "content_sha256": _content_sha256(p) if not p.is_dir() else None,
    }
...
def emit_lakehouse_contracts(
    ...
    run_manifest = {
        ...
        "env": {
            "MBAL_ACCEL": os.environ.get("MBAL_ACCEL"),
            "MBAL_BATCH_SIZE": os.environ.get("MBAL_BATCH_SIZE"),
            "MBAL_WINDOWS_BATCH": os.environ.get("MBAL_WINDOWS_BATCH"),
            "MBAL_INFER_WINDOWS_BATCH": os.environ.get("MBAL_INFER_WINDOWS_BATCH"),
            "MBAL_VALID_BATCH": os.environ.get("MBAL_VALID_BATCH"),
        },
        "params": config,
        "drivers": summary.get("drivers"),
        "artifacts": {
...
def _build_long_frame(smoke: bool) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Physics-guided spectral features: explicit tidal harmonics (M2 and K1)
    hours_continuous = (idx - pd.Timestamp("2000-01-01", tz="UTC")).total_seconds() / 3600.0
    cal["m2_tide_sin"] = np.sin(2 * np.pi * hours_continuous / 12.42)
    cal["m2_tide_cos"] = np.cos(2 * np.pi * hours_continuous / 12.42)
    cal["k1_tide_sin"] = np.sin(2 * np.pi * hours_continuous / 23.93)
    cal["k1_tide_cos"] = np.cos(2 * np.pi * hours_continuous / 23.93)
...
# the library itself uses to validate exog inputs).
EXOG_CAPS = {
    "nhits":        {"futr": True,  "hist": True,  "stat": True},
    "patchtst":     {"futr": False, "hist": False, "stat": False},
    "mbal_moe":    {"futr": False, "hist": False, "stat": False},
    ...
    # "mbal_recursive" removed (P1): MBAL_Recursive_Transformer is not defined in
    # mbal_deep_models — re-add here + the class + a test before advertising it again.
}


from neuralforecast.common._base_model import BaseModel

class MBALNeuralWrapper(BaseModel):
    """
    Adapter to make MBAL-specific models (MoE, Recursive) compatible with NeuralForecast.
    """
    def __init__(self, h, input_size, mbal_model_type, semantic_info, 
                 recursive_depth=1, **kwargs):
        from neuralforecast.losses.pytorch import MAE
        # Provide defaults for mandatory BaseModel arguments if missing
        kwargs.setdefault('loss', MAE())
        kwargs.setdefault('valid_loss', MAE())
        kwargs.setdefault('learning_rate', 1e-3)
        kwargs.setdefault('max_steps', 1000)
        kwargs.setdefault('val_check_steps', 100)
        kwargs.setdefault('batch_size', 32)
        kwargs.setdefault('valid_batch_size', 32)
        kwargs.setdefault('windows_batch_size', 1024)
        kwargs.setdefault('inference_windows_batch_size', 1024)
        kwargs.setdefault('start_padding_enabled', False)
        
        super().__init__(h=h, input_size=input_size, **kwargs)
        self.mbal_model_type = mbal_model_type
        self.semantic_info = semantic_info
        self.recursive_depth = recursive_depth
        self.mbal_model = None

    def forward(self, windows_batch):
        insample_y = windows_batch['insample_y']
        # Take the last time point as representative features for the flat models
        if insample_y.dim() == 3:
            x = insample_y[:, -1, :]
        else:
            x = insample_y[:, -1].unsqueeze(-1)
            
        if self.mbal_model is None:
            from mbal_deep_models import MBAL_MoE_Transformer
            n_feat = x.shape[1]
            if self.mbal_model_type == 'moe':
                self.mbal_model = MBAL_MoE_Transformer(
                    n_features=n_feat, semantic_info=self.semantic_info,
                    num_experts=8
                )
            else:
                raise ValueError(
                    f"unsupported mbal_model_type={self.mbal_model_type!r}; "
                    "only 'moe' is implemented ('recursive' has no backing class)")
            self.mbal_model = self.mbal_model.to(x.device)
            
        out = self.mbal_model(x)
        # Map to [B, H, 1] (or out_features)
        if out.dim() == 2:
            out = out.unsqueeze(1).repeat(1, self.h, 1)
        return out


def make_model(name: str, h: int, input_size: int, max_steps: int,
               loss_kind: str = "mae", futr_extra: list[str] | None = None,
               hist_cols: list[str] | None = None, recursive_depth: int = 1,
               semantic_info: list = None, logfn=print):
    import os
    from neuralforecast.models import NHITS, PatchTST, TFT, NBEATSx, TSMixerx, iTransformer
    ...
    def _bi(name, default):
        try:
            ...
            return default
    bounded = dict(batch_size=_bi("MBAL_BATCH_SIZE", 24),
                   windows_batch_size=_bi("MBAL_WINDOWS_BATCH", 256),
                   inference_windows_batch_size=_bi("MBAL_INFER_WINDOWS_BATCH", 512),
                   valid_batch_size=_bi("MBAL_VALID_BATCH", 256),
                   dataloader_kwargs=dict(num_workers=0, pin_memory=False))
    accel = os.environ.get("MBAL_ACCEL", "auto")  # cpu|gpu|auto -- gpu hard-crashes (sm_120/torch2.11) at full scale
    if accel != "auto":
    ...
    base = dict(h=h, input_size=input_size, max_steps=max_steps, scaler_type="standard",
                loss=loss, valid_loss=loss, learning_rate=1e-3, val_check_steps=100,
                start_padding_enabled=False,
                random_seed=42, enable_progress_bar=False, **bounded)
    exog = {}
    ...
    if n == "tft":
        return TFT(**base, **exog)
    if n == "nbeatsx":
        return NBEATSx(**base, **exog)
    if n == "tsmixerx":
        return TSMixerx(n_series=common_n_series, **base, **exog)
    if n == "itransformer":
        return iTransformer(n_series=common_n_series, **base)
    if n == "mbal_moe":
        return MBALNeuralWrapper(mbal_model_type=n.replace("mbal_", ""),
...
def main() -> None:
    ...
    ap.add_argument("--recursive-depth", type=int, default=1, help="Depth of iterative refinement for recursive models")
    a = ap.parse_args()

    # GPU admission guard disabled - forcing execution with existing VRAM (user override)
    if False:
        enforce_gpu_admission(
            label=f"mbal_neural_forecast.py model={a.model}",
            request_mib=estimate_neural_forecast_mib(
                model=a.model,
                smoke=a.smoke,
                loss=a.loss,
                drivers=bool(a.drivers_parquet or a.drivers_manifest),
            ),
            enabled=os.environ.get("MBAL_ACCEL", "auto").lower() != "cpu",
        )
    ...
    # Identify feature columns (all columns except reserved ones)
    reserved_cols = {"unique_id", "ds", "y"}
    feature_cols = [c for c in long_df.columns if c not in reserved_cols]
    ...
    from mbal_deep_models import parse_mbal_feature_semantics  # P1 fix: was unimported in main()
    semantics = parse_mbal_feature_semantics(feature_cols)
    model = make_model(a.model, h, input_size, max_steps,
                       loss_kind=a.loss, futr_extra=futr_extra, hist_cols=hist_cols, 
                       recursive_depth=a.recursive_depth, semantic_info=semantics, logfn=P)
