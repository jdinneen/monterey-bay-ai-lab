...
# root (not the script dir) when MBARI_PROJECT_ROOT is unset, so nn_results resolves.
PROJECT_ROOT = Path(os.environ.get("MBARI_PROJECT_ROOT", Path(__file__).resolve().parents[2])).resolve()
ROOT = PROJECT_ROOT / "nn_results"
XGB = PROJECT_ROOT / "mbari_forecast_v2_results_nodrivers" / "leaderboard.csv"
...
def main() -> None:
    ...
    L = ["# MBARI Neural Forecasting — Unified Leaderboard", "",
