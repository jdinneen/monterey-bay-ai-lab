...
def render_report(reg: dict, repo_root: Path | None = None) -> str:
    ...
    lines += [
        ...
        "`bacteria_hgbt_isotonic` remains THE fundable, statewide headline: a calibrated "
        "enterococcus nowcast that beats deployed practice and — verified by leave-one-beach-out "
        "(AP 0.494 / ROC-AUC 0.852) — generalizes to beaches it never trained on. (A *learned "
        "spatial surface* from raw coordinates was tested and is a WASH, not a lift; see the "
        "`bacteria_hgbt_spatial` negative-result row.) A second, distinct claimable now exists on "
        "the marine-HAB frontier: `da_forecast_hgbt` (domoic-acid), which beats best-naive 8/8 "
        "LOSO stations and the operational NOAA C-HARM nowcast 8/8 piers (AP 0.271 vs 0.027; "
        "evidence `reports/hab/charm_comparison.json`, a frozen live-ERDDAP comparison). It "
        "reproduces from the shipped 304 KB CalHABMAP input, but rests on a small 52-event test "
        "set pending a prospective pilot, so it is a frontier result, not the statewide headline. "
        "Everything on the M1-forecasting / neural side remains a benchmark, research-only, or a "
        "documented negative.",
    ]
    if errs:
