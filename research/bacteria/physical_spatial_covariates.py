...
spatial_drivers_experiment.py showed raw lat/lon (a black-box spatial surface) lifts AP and
reduces residual Moran's I, and generalises to never-seen *beaches*. But two things it CANNOT
do: (a) explain *why* a beach is risky, and (b) extrapolate to a region with no nearby training
beaches (lat/lon can only interpolate inside the training spatial envelope). Physical
covariates -- distance to the nearest freshwater discharge point, local monitoring density as a
proxy for urbanisation/embayment -- can do both.
