...
def load_timed_station_days(timed_path: Path, label: str = "enterococcus") -> pd.DataFrame:
    ...
    Expected input is a source-neutral timed-label parquet: one row per analyte
    result with `record_status`, `analyte`, `value`, `sampledate`, `station_id`,
    `county`, and `available_at`.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    (out_dir / "operational_benchmark.json").write_text(json.dumps(res, indent=2), encoding="utf-8", newline="\n")
    (out_dir / "operational_benchmark.md").write_text(to_markdown(res), encoding="utf-8", newline="\n")
