...
on beaches never seen in training, the San-Diego-excluded driver model holds at **AP 0.494 / ROC-AUC
0.852 / ECE 0.012** — within noise of the temporal-holdout headline — and still beats station
memory (AP 0.278) and AB411 (AP 0.178), so the skill is a transferable environmental pattern, not
an artifact of co-located stations leaking across folds. **However**, Moran's I on the per-beach
residuals is **0.20 (one-sided p=0.001; expected ~0)** San-Diego-excluded, i.e. residuals remain
spatially clustered: there is genuine sub-county spatial structure the current features do not
capture, motivating explicit spatial / hierarchical features. Harness:
`research/bacteria/spatial_autocorr.py` (unit-tested in `tests/test_spatial_autocorr.py`).

**Acting on (4): does a learned spatial surface help? (No — it is a wash.)** We tested whether
adding raw station coordinates (latitude, longitude) or a causal k-nearest-beach exceedance lag
lets the gradient-boosted model learn a spatial risk surface directly. On the San-Diego-excluded
enterococcus holdout it is a **wash**: every spatial arm — `+latlon`, `+nbr_lag`, `+nbr&latlon` —
scores **AP 0.497 / ROC-AUC 0.859 / ECE 0.008**, identical to the no-spatial driver model, and
residual Moran's I is **unchanged at 0.228**. The reason is that the model's per-station
(`station_prior_rate`) and county/statewide rate features already encode location, so raw
coordinates add no separable information the trees can exploit. *(An earlier draft reported a
+0.033 lift here; that figure does not reproduce on the committed pipeline. The cause was a
merge-collision in `spatial_drivers_experiment.py` that silently NaN-filled the coordinate columns
(`latitude_x`/`latitude_y`), masking the true result; with that fixed the arms are a clean wash. We
report the reproducible result, not the stale one.)* The residual clustering (Moran's I 0.20,
~9 sigma above the permutation null, p=0.001) confirms there IS unmodelled sub-county spatial
structure — so **physical spatial covariates (outfall / river-mouth distance, embayment)**, not
raw coordinates, remain the next lever. Harness: `research/bacteria/spatial_drivers_experiment.py`
(tests `tests/test_spatial_drivers.py`).
