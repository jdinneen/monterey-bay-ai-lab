...
def evaluate_county(df: pd.DataFrame, feats: list[str], county: str, clf_factory) -> dict | None:
    ...
    clf = clf_factory()
    clf.fit(train[feats].astype(float), train["exceed"].to_numpy())
    raw = clf.predict_proba(test[feats].astype(float))[:, 1]
    p_cal = raw
    if cal["exceed"].nunique() > 1:
        from sklearn.isotonic import IsotonicRegression

        iso = IsotonicRegression(out_of_bounds="clip")
        iso.fit(clf.predict_proba(cal[feats].astype(float))[:, 1], cal["exceed"].to_numpy())
...
def main(argv: list[str] | None = None) -> int:
    ...
    (out_dir / "spatial_holdout.json").write_text(json.dumps(res, indent=2), encoding="utf-8")
    (out_dir / "spatial_holdout.md").write_text(to_markdown(res), encoding="utf-8")
