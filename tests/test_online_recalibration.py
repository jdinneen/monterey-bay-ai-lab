...
def test_run_online_recal_propagates_reveal_lag_to_features(tmp_path, monkeypatch):
    monkeypatch.delenv("MBAL_BACTERIA_OBS", raising=False)
    real_add = orc.ob.add_causal_features
    seen: list[int] = []

    def wrapped(df, reveal_lag_days=0):
        seen.append(reveal_lag_days)
        return real_add(df, reveal_lag_days=reveal_lag_days)

    monkeypatch.setattr(orc.ob, "add_causal_features", wrapped)

    orc.run_online_recal(_multi_county_obs(tmp_path), clf=_tiny_clf(), lag_days=5, min_calib=20)
