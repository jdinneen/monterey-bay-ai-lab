#!/usr/bin/env python3
"""
Unit tests for the MBARI training orchestrator (mbari_train.py).
...
Run with pytest if available, else as a plain script (asserts):
    python -m pytest tests/test_mbari_train.py -q
    python tests/test_mbari_train.py
"""
...
import mbari_train as mt  # noqa: E402
...
def test_safe_profile_values():
    ...
    assert mt.BATCH_PROFILES["safe"] == {
        "MBARI_BATCH_SIZE": 12, "MBARI_WINDOWS_BATCH": 64,
        "MBARI_INFER_WINDOWS_BATCH": 128, "MBARI_VALID_BATCH": 64}
