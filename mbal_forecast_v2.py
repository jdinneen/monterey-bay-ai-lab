#!/usr/bin/env python3
"""
Monterey Bay AI Lab M1 multi-signal time-series FORECASTING harness (v2, leakage-corrected).

This rewrite fixes the methodological problems in the earlier modeling passes:

1. LEAKAGE / "no horizon" bug (see mbal_analysis_results/MBAL_STATISTICAL_ANALYSIS.md):
...
5. DATA SOURCE is config driven: --source bq (BigQuery via load_mbal_data) or
...
Reuses load_mbal_data, apply_physical_quality_filters, add_derived_features, detect_compute,
PROJECT/DATASET/TABLE ids, RAW_COLUMNS and PHYSICAL_RANGES from mbal_core.py.

Usage examples:
    python mbal_forecast_v2.py --smoke                       # fast, capped, auto data source
    python mbal_forecast_v2.py --source bq                   # full BigQuery run
    python mbal_forecast_v2.py --source parquet --path data  # historical parquet dir/file
"""
...
OUT_DIR = PROJECT_ROOT / "mbal_forecast_v2_results"
SMOKE_OUT_DIR = PROJECT_ROOT / "mbal_forecast_v2_smoke_results"

# Default parquet path (matches mbal_neural_forecast.py)
DEFAULT_PARQUET_PATH = str(PROJECT_ROOT / "mbal_history" / "opendap")
...
# Expanded to 168h to encompass a full synoptic weather cycle (Google AI Roadmap Step 3).
LAGS_HOURS = [1, 2, 3, 6, 12, 24, 48, 72, 120, 168]
# Rolling-window sizes (hours) for causal mean/std (computed on shift(1) history).
ROLL_WINDOWS_HOURS = [6, 24, 72, 168]
...
def load_source(source: str, path: str | None, limit: int | None) -> pd.DataFrame:
    ...
    """
    if source == "bq":
        df = load_mbal_data(limit=limit)
    elif source == "parquet":
        if not path:
            path = os.environ.get("MBAL_SOURCE_PARQUET", DEFAULT_PARQUET_PATH)
        p = Path(path)
        if p.is_dir():
...
def build_hourly_matrix(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Using continuous hours from the start of the epoch to preserve absolute phase
    hours_continuous = (idx - pd.Timestamp("2000-01-01", tz="UTC")).total_seconds() / 3600.0
    matrix["m2_tide_sin"] = np.sin(2 * np.pi * hours_continuous / 12.42)
    matrix["m2_tide_cos"] = np.cos(2 * np.pi * hours_continuous / 12.42)
    matrix["k1_tide_sin"] = np.sin(2 * np.pi * hours_continuous / 23.93)
    matrix["k1_tide_cos"] = np.cos(2 * np.pi * hours_continuous / 23.93)
...
# at their concurrent value. Includes physics-guided tidal harmonics.
TIME_COLS = [
    "hour_sin", "hour_cos", 
    "doy_sin", "doy_cos", 
    "days_since_start",
...
def deep_forecast_hook(
    ...
    try:
        from mbal_deep_models import chronos_forecast, patchtst_forecast
...
def fit_target_horizon(
    ...
    for k, (train_end, test_start, test_end) in enumerate(folds):
        ...
        deep_attr = None
        if use_deep_hook:
            hook_res = deep_forecast_hook(
                x_train, tr["__y_future"], x_test, horizon, y_history=matrix[target_col]
            )
        ...
        # --- Physical Clamping (Metric Hygiene) ---
        p_min, p_max = PHYSICAL_RANGES.get(variable, (-np.inf, np.inf))
        model_pred = np.clip(model_pred, p_min, p_max)
        if q_lo is not None:
            q_lo = np.clip(q_lo, p_min, p_max)
            q_hi = np.clip(q_hi, p_min, p_max)
...
def write_outputs(
    ...
    lines = [
        "# Monterey Bay AI Lab M1 Forecasting v2 (leakage-corrected)",
        "",
    ...
    is_historical_parquet = source_text == "parquet" and "mbal_history" in path_text.replace("\\", "/")
    ...
    if is_historical_parquet:
        limitation_lines.append(
            "- Historical M1 coverage is now multi-year, but current velocity exists only where legacy CMSTV products are available; models using currents must account for that sparse feature family."
        )
        limitation_lines.append(
            "- Seasonal and 72h horizons are now plausible to evaluate, but they still require target-specific review against persistence across folds before reporting."
        )
    else:
        limitation_lines.append(
...
def run(config: dict[str, Any]) -> None:
    ...
    if source == "auto":
        for cand in ("bq", "parquet", "synthetic"):
            try:
...
def parse_args() -> dict[str, Any]:
    ap = argparse.ArgumentParser(description="Monterey Bay AI Lab M1 leakage-corrected forecasting harness.")
