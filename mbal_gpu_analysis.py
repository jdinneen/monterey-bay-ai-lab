#!/usr/bin/env python3
"""
Robust statistical analysis for MBAL M1 BigQuery data.
...
# GCP project is environment-configured (set MBAL_GCP_PROJECT) so no specific
# cloud project is baked into the published source.
PROJECT_ID = os.environ.get("MBAL_GCP_PROJECT", "your-gcp-project")
DATASET_ID = "blue_current_raw"
TABLE_ID = "mbal_m1_realtime"
OUT_DIR = Path("mbal_analysis_results")
...
def load_mbal_data(limit: int | None = None) -> pd.DataFrame:
...
# (fit_target_model removed; see mbal_forecast_v2.py)
...
def write_markdown_report(
    ...
    (OUT_DIR / "MBAL_STATISTICAL_ANALYSIS.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    print(f"Analysis complete. Report: {OUT_DIR / 'MBAL_STATISTICAL_ANALYSIS.md'}")
