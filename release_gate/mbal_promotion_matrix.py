#!/usr/bin/env python
"""Build an enforceable promotion matrix for MBAL forecast models.
...
def _read_xgb_baseline(project_root: Path) -> pd.DataFrame:
    ...
    if not lakehouse.empty:
        return lakehouse
    path = project_root / "mbal_forecast_v2_results" / "leaderboard.csv"
    if not path.exists():
        return pd.DataFrame()
    df = pd.read_csv(path)
    ...
    out["xgb_run_id"] = (
        out["run_id"].astype(str) if "run_id" in out.columns else "mbal_forecast_v2_results"
    )
    out["xgb_split_id"] = (
...
def write_outputs(matrix: pd.DataFrame, project_root: Path, output_dir: Path) -> dict[str, str]:
    ...
    try:  # Phase-1 shadow dual-write — opt-in via MBAL_GOLD_SHADOW_DELTA=1, non-fatal
        ...
        shadow_dual_write("promotion_matrix", matrix, gold_dir=parquet_path.parent.parent)
    except Exception:
        pass
...
def make_markdown(matrix: pd.DataFrame, summary: dict[str, Any]) -> str:
    lines = [
        "# Monterey Bay AI Lab Promotion Matrix",
        "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Monterey Bay AI Lab model promotion matrix.")
