#!/usr/bin/env python
"""Release gate for the local MBARI AI analysis setup.
...
def check_historical_parquet(project_root: Path) -> Check:
    parquet_paths = sorted(project_root.glob("mbari_history/**/*.parquet"))
    details: dict[str, Any] = {"files": [str(path) for path in parquet_paths]}
    if not parquet_paths:
        return Check(
            "historical_parquet",
            "WARN",
            "no historical Parquet files found under mbari_history",
            details,
        )
...
def check_curated_dataset(project_root: Path) -> Check:
    dataset_dir = project_root / "mbari_pipeline" / "curated_history"
    report_path = project_root / "mbari_pipeline" / "reports" / "history_validation.json"
    details: dict[str, Any] = {
...
def check_model_result_file(path: Path) -> Check:
    ...
    try:
        ...
        is_production_result = path.parent.name == "mbari_forecast_v2_results"
        if is_production_result:
            ...
            if not str(run_config.get("path", "")).replace("\\", "/").endswith("mbari_history/opendap/m1_history.parquet"):
                statuses.append("FAIL")
...
def check_model_outputs(project_root: Path) -> Check:
    result_paths = sorted(project_root.glob("mbari*_results/model_results.json"))
    details: dict[str, Any] = {"files": [str(path) for path in result_paths]}
    if not result_paths:
        return Check(
            "model_outputs",
            "WARN",
            "no MBARI model_results.json files found",
            details,
        )
    production_paths = [path for path in result_paths if path.parent.name == "mbari_forecast_v2_results"]
...
def check_promotion_matrix(project_root: Path) -> Check:
    ...
    if not matrix_path.exists():
        return Check(
            "promotion_matrix",
            "WARN",
            "promotion matrix is missing; run release_gate/mbari_promotion_matrix.py",
            details,
        )
    try:
        import pandas as pd
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the MBARI AI release gate.")
    parser.add_argument(
        "--project-root",
        type=Path,
