...
- **Verified margin:** on a statewide California enterococcus benchmark (1.32M lab **assays** → ~104k modeled station-days; test events ≥2022), a calibrated gradient-boosted model reaches **ROC-AUC 0.858 / AP 0.497** on the **San-Diego-excluded** stratum — the honest headline, since the cross-border San Diego/Tijuana plume is a 2022 data-regime artifact and is reported separately, not pooled. It beats the deployed AB411 rain rule (AP 0.18) and a Virtual-Beach-class MLR (AP 0.38), is well-calibrated after isotonic calibration (ECE 0.020 at the operational 2-day lab-result reveal lag — the tracked, byte-reproducible `expected/` outputs; see `MODEL_SUITE.md`), and generalizes spatially: it beats both the regulatory rule and station-memory in **9/9 leave-one-county-out folds**. Decision-support-grade. (The older "AUC 0.888" headline was the artifact-inflated pooled/multi-analyte figure and has been retired — see `research/bacteria/reproduce/PAPER_DRAFT.md`, which derives the honest San-Diego-excluded stratum and the reveal-lag-enforced metrics above.)
- **The honest frontier:** site memory is a strong baseline (a recently-dirty beach tends to stay dirty; station-memory alone reaches AUC ~0.77 / AP 0.28 on the same San-Diego-excluded stratum). The model already beats it meaningfully (+0.22 AP) with environmental drivers; the technical-risk frontier is widening that margin and extending it to beaches we **don't** monitor. First real step: a **learned spatial risk surface** (station coordinates) lifts AP **0.46→0.49** and — verified — **generalizes to never-trained-on beaches** (leave-one-beach-out +0.034 AP, stable across 5-fold leave-one-beach-out, not a county proxy), the basis for risk estimates at unmonitored sites. It is rainfall-conditioned, and residual spatial autocorrelation is reduced but not eliminated (Moran's I 0.24→0.20, still significant), so physical spatial covariates (outfall / river-mouth distance) are the next lever. See `research/bacteria/reproduce/PAPER_DRAFT.md` and `research/bacteria/spatial_drivers_experiment.py`.

### 2. Foundational Mooring Physics (M1 / M2) — the backbone
21.6 continuous years of hourly M1/M2 mooring data (temperature, salinity, synoptic met) — the longest, cleanest, QC'd, release-gate-blessed asset in the lab. It anchors the science and supplies physical drivers.
...
We leverage an **RTX 5090 cluster** for intensive iterative testing.
...
- **Geolocation Unblock:** Successfully mapped **99.75% of statewide stations** to verified coordinates (`station_geo.parquet`), enabling high-precision gridded joins.
...
- **The Forge:** All high-performance training and data normalization happens on our local RTX 5090 cluster. This environment is private; no inbound traffic is allowed.
