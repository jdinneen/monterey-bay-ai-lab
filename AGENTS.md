...
**Do not call this project "MBARI", "MBARI AI", "MBARI AI Lab", or "MBARI AI Forecasting".**
MBAL is acceptable as shorthand or as an internal/legacy code token — a URL/domain,
module/file name, or compatibility environment variable such as `MBAL_LAKEHOUSE_DIR`.
It is NOT a data provider. MBARI may appear only when factually naming the source
provider for M1/M2 mooring data. User-facing dashboards, GitHub metadata, package
metadata, public docs, generated reports, and agent summaries should introduce the
project as **Monterey Bay AI Lab**.
...
- **Lightweight GPU Optimization:** Do not reserve more VRAM just to "go faster." `ops/run_safe.py` prints a small optimization check for GPU jobs; follow it. For long runs, use a watchdog and prefer a short measured probe (`--max-steps`, larger `--batch-rows`, or the runner's existing tuning flags) before increasing `--gpu-mib`. Keep this as simple measurement, not a new framework.
...
(+0.19 AP) and the fundable frontier is **widening that margin with environmental
