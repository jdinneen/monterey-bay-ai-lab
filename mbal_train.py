#!/usr/bin/env python3
"""
Monterey Bay AI Lab neural-forecast training ORCHESTRATOR (single source of truth).

This wraps the stable trainer ``mbal_neural_forecast.py`` (one model per invocation,
...
DO NOT import this from / modify mbal_neural_forecast.py -- the trainer is treated as a
...
from ops.gpu_admission import enforce_gpu_admission, estimate_mbal_train_mib

# --------------------------------------------------------------------------------------
# Paths & constants (match mbal_neural_forecast.py)
# --------------------------------------------------------------------------------------
ROOT = Path(os.environ.get("MBAL_PROJECT_ROOT", Path(__file__).resolve().parent)).resolve()
TRAINER = ROOT / "mbal_neural_forecast.py"
CACHE_DIR = ROOT / "nn_cache"
RESULTS_DIR = ROOT / "nn_results"
LOCK_PATH = CACHE_DIR / ".gpu.lock"
MANIFEST_PATH = RESULTS_DIR / "run_manifest.json"
CACHE_VERSION = "v4_physics_guided_tidal_futr"  # P1 fix: sync with mbal_neural_forecast.CACHE_VERSION

PYTHON = os.environ.get("MBAL_PYTHON", sys.executable)
...
BATCH_PROFILES: dict[str, dict[str, int]] = {
    "safe":   {"MBAL_BATCH_SIZE": 12, "MBAL_WINDOWS_BATCH": 64,  "MBAL_INFER_WINDOWS_BATCH": 128, "MBAL_VALID_BATCH": 64},
    "medium": {"MBAL_BATCH_SIZE": 16, "MBAL_WINDOWS_BATCH": 128, "MBAL_INFER_WINDOWS_BATCH": 256, "MBAL_VALID_BATCH": 128},
    "large":  {"MBAL_BATCH_SIZE": 24, "MBAL_WINDOWS_BATCH": 256, "MBAL_INFER_WINDOWS_BATCH": 512, "MBAL_VALID_BATCH": 256},
}
...
def _subprocess_runner(cmd, env, raw_log: Path, stall_timeout_s: float, poll_s: float,
    ...
    with open(raw_log, "w", encoding="utf-8", errors="replace") as fh:
        proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                                text=True, bufsize=1, env=env, **trainer_popen_kwargs())
        ...
        reader = threading.Thread(target=read_stdout, name="mbal-train-log-reader", daemon=True)
        reader.start()
        
        from ops.mbal_sota_utils import SafetyMonitor
        safety_monitor = SafetyMonitor(vram_threshold=0.80, max_runtime_hours=72.0, idle_shutdown_minutes=30.0)
        ...
        try:
            while True:
                ...
                if reasons:
                    logfn(f"[safety] SHUTDOWN: {', '.join(reasons)} -> killing")
                    terminate_process_tree(proc)
                    return RunOutcome(exit_code=-1, reason="safety_shutdown", minutes=(time.time() - t0) / 60)
...
def load_jobs(jobs_path: Optional[str]) -> list[Job]:
    ...
    return [Job(label=j["label"], model=j["model"], outdir=j["outdir"],
                extra_args=list(j.get("extra_args", j.get("args", []))), profile=j.get("profile", "safe"))
            for j in items]
...
def run_sweep(jobs: list[Job], args, logfn: Callable[[str], None] = print) -> list[JobResult]:
    ...
    with WakeLock(logfn=logfn, enabled=not args.no_wake_lock):
        with GpuLock(label="mbal_train sweep", logfn=logfn) as lock:
            ...
            for job in jobs:
                ...
                if args.ewc:
                    job_to_run.extra_args.append("--ewc")
                if args.checkpoint:
                    job_to_run.extra_args.extend(["--checkpoint", args.checkpoint])
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Robust orchestrator for Monterey Bay AI Lab NeuralForecast training (single source of truth).")
    ...
    ap.add_argument("--ewc", action="store_true", help="trigger EWC logic during training")
    ap.add_argument("--checkpoint", type=str, default=None, help="prior model checkpoint for EWC incremental updates")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # (ops/train.ps1, direct `python mbal_train.py`, the detached launcher).
    ...
    if args.accel != "cpu" and not args.no_gpu_preflight:
        enforce_gpu_admission(
            label=f"mbal_train.py sweep profile={args.profile or 'safe'}",
            request_mib=estimate_mbal_train_mib(args.profile),
        )

    print(f"=== mbal_train sweep start {time.strftime('%Y-%m-%dT%H:%M:%S')} === ({len(jobs)} jobs)")
    results = run_sweep(jobs, args)
    write_manifest(results, meta=_meta(args))
    done = sum(1 for r in results if r.status == "done")
    skipped = sum(1 for r in results if r.status == "skipped")
    failed = sum(1 for r in results if r.status == "failed")
    print(f"=== mbal_train sweep end === done={done} skipped={skipped} failed={failed}")
